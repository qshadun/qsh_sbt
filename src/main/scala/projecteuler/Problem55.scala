...
Lychrel numbers
Problem 55
Published on Friday, 24th October 2003, 06:00 pm; Solved by 23931
...
object Problem55 {
  def main(args: Array[String]): Unit = {
    def isPalindrome(n: BigInt) = n.toString.reverse == n.toString
    def isLychrel(n: Int) = {
      def lych(num: BigInt) = num + BigInt(num.toString.reverse)
      def recur(num: BigInt, count: Int): Boolean = 
        if (isPalindrome(num)) false
        else 
          if (count > 50) true else recur(lych(num), count + 1) 
      recur(lych(n), 1)
    }
    println((1 to 10000).filter(isLychrel).size)
  }
}