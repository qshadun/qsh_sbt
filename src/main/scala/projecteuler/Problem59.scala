...
object Problem59 {
  def main(args: Array[String]): Unit = {
    val input = Source.fromURL(getClass.getResource("/cipher1.txt")).mkString.trim.split(',').map(_.toInt)
    val cs = 'a'.toInt to 'z'.toInt
    def decode(source: Seq[Int], key: Seq[Int]): Seq[Int] = {
      source.grouped(3).map{x =>
        x.zip(key).map(y => y._1 ^ y._2)
      }.flatten.toSeq
    }
    for(k1 <- cs; k2 <- cs; k3 <- cs; key = List(k1, k2, k3))
    {
      val decoded = decode(input, key)
      if (decoded.map(_.toChar).mkString.split(' ').exists(_=="the")) {
        println(key.map(_.toChar).mkString + "," + decoded.map(_.toChar).mkString)
        println(decoded.sum)
      }
    }
  }
